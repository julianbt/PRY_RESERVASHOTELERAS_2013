...
class EmpleadoController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Persona']))
		{
            ...
            if(isset($_FILES) and $_FILES['Persona']['error']['hoja_vida']==0)
            {
                $uf = CUploadedFile::getInstance($model, 'hoja_vida');
                if($uf->getExtensionName() == "jpg" || $uf->getExtensionName() == "png" ||
                    $uf->getExtensionName() == "jpeg" || $uf->getExtensionName()== "gif")
                {
                    $uf->saveAs(Yii::getPathOfAlias('webroot').'/uploads/'.$uf->getName());
                    $model->hoja_vida = '/uploads/'.$uf->getName();
                    if($model->save())
                        $this->redirect(array('view','id'=>$model->id));
                    #$this->refresh();
                }else{
                    Yii::app()->user->setFlash('error_imagen','Hoja vida no valida');
                }
            }
            /*
			if($model->save())
